...
def make_test_database(settings):
    try:
        ...
        MySQL.execute_file(settings.perftest, "tests/resources/sql/schema_perftest.sql")
        MySQL.execute_file(settings.perftest, "tests/resources/sql/Add test_data_all_dimensions.sql")

        Log.note("MIGRATE {{database}} TO NEW SCHEMA", {"database": settings.perftest.schema})
        MySQL.execute_file(settings.perftest, "resources/migration/alerts.sql")
        MySQL.execute_file(settings.perftest, "resources/migration/v1.2 email.sql")
        ...
        MySQL.execute_file(settings.perftest, "tests/resources/sql/add_objectstore.sql")
        MySQL.execute_file(settings.perftest, "tests/resources/sql/json.sql")
