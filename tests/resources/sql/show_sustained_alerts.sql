...
select 
	...
	round((json.number(json.json(details, 'future_stats'), 'mean') - json.number(json.json(details, 'past_stats'), 'mean'))/json.number(json.json(details, 'past_stats'), 'mean'), 2) diff,
