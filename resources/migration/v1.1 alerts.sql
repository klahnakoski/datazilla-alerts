  ...
  BEGIN
    ...
    THEN
      ...
      THEN
        ...
        SET @util_curr_id = 0;
      END IF;
      UPDATE util_uid_next
      SET id=@util_curr_id + 1000;
    ELSEIF mod(@util_curr_id, 1000) = 0
      THEN
        SELECT
          max(id)
        INTO @util_curr_id
...
CREATE PROCEDURE get_version(
  ...
  WHERE table_schema = schema_name AND table_name = 'database';
  IF (is_version_1 = 0)
  THEN
...
CREATE PROCEDURE `migrate v1.1`()
    ...
    INSERT INTO alert_stati VALUES ('obsolete');
